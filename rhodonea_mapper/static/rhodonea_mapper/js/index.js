...
class DrawerForm {
  ...
  validateForm(submitting=false) {
    ...
    if(submitting === true) {
      ...
      if (input.val() === "") { this.makeInputInvalid(input); }
      else { this.makeInputValid(input); }
    }
    ...
    try {
      ...
      for(let f of features) {
        ...
        for(let key of Object.keys(this.DEFAULTS)) {
          ...
          if(key === 'strokeColor') { payloadKey = 'stroke_color'; }
          else if(key === 'strokeWeight') { payloadKey = 'stroke_weight'; }
